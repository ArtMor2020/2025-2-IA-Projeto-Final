...
class InferenceEngine:
    ...
    def update_history(self, rule_id, success, penalty_factor=0.1):
        ...
        if success:
            ...
            # permite adicionar valores fracionários (penalização leve)
            self.history[rid]["fail"] = self.history[rid].get("fail", 0) + penalty_factor

# Paginação para a UI
def paginate_list(items, page_size):
    """Retorna o total de páginas e função de acesso por página."""
...
class ConsoleUI:
    ...
    def consult(self):
        ...
        symptoms = self.persistence.kb["symptoms"]
        page_size = 30
        page = 0
        total_pages = (len(symptoms) - 1) // page_size + 1
        ...
        while True:
            ...
            if any(c.isdigit() for c in choice):
                try:
                    user_symptoms = [int(x.strip()) for x in choice.split(",") if x.strip().isdigit()]
                    break
                except:
        ...
        sol_page_size = 10
        sol_page = 0
        sol_total_pages = (len(matches) - 1) // sol_page_size + 1
        ...
        while selected_solution is None:
            ...
            start = sol_page * sol_page_size
            end = start + sol_page_size
            ...
            print(f"\nPágina {sol_page+1}/{sol_total_pages}")
        ...
        if selected_solution:
            ...
            # Penalização leve nas não escolhidas
            for m in matches:
                if m["rule_id"] != selected_solution["rule_id"]:
                    self.engine.update_history(m["rule_id"], False, penalty_factor=0.1)

            print("\nFeedback registrado: solução escolhida recebeu reforço positivo.")
            input("Pressione ENTER para continuar.")
        else:
            # Penalização padrão em todas
            for m in matches:
                self.engine.update_history(m["rule_id"], False)

            print("\nFeedback registrado: nenhuma solução válida, penalização aplicada a todas.")
            input("Pressione ENTER para continuar.")
    ...
    def add_rule(self):
        ...
        add_new = input("\nDeseja adicionar novos sintomas? (s/n): ").lower()
        new_symptom_ids = []
        if add_new == "s":
            while True:
        ...
        symptom_ids += new_symptom_ids  # inclui os novos
...
# KB expanded
# Solutions are no longer duplicated in default kb
# Paging added
# Added CLEAR to outputs, linebreaks fixed